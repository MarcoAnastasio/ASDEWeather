...
function ForecastWeatherDecoder(forecastWeather_input) {
	...
	var date = new dateDecoder(forecastWeather_input.dateTimeCalulation);
	this.date = date;
...
function dateDecoder(date_input) {
	...
	var hour = times[0];
	var minute = times[1];
	var day_name = DayNamer(month + "/" + day + "/" + year);
...
function CityDecoder(city_input){
	...
	this.country=new CountryDecoder(city_input.country);
	 
	
}
function CountryDecoder(country_input){
