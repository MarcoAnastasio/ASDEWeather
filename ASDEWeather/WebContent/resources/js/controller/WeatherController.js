 ...
 * 
 */
App.controller("WeatherController",["$scope","$rootScope","$localStorage","$sessionStorage", 
	...
	$scope.loadOneCity  = function(){
		...
		$.ajax({
			type:'POST',
			url:"ASDEWeather/api/user/forecastWeatherByCity", 
			contentType:"application/json",
			dataType:"json",
			data:JSON.stringify(dataToSend),
			success:function(response,status){
...
//			}
	...
	/*
	 *  Adding cities to my city
	 * 
	 */


	$scope.AddToMyCities = function(cityName) {
		...
		console.log("Adding city" + cityName +" To user : "+ + user_id +"," + user_username + "preferences");
